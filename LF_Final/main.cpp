...
void handleClient(int client_fd, ThreadPool& pool) {
    RequestHandling requestHandler(graph);
    requestHandler.processClient(client_fd,pool);
}
...
int main() {
    ...
    while (true) {
        ...
        if (client_fd != -1) {
            // Enqueue client command handling
                handleClient(client_fd,pool); // Handle client requests and enqueue commands
