...
void pipeline() {
    ...
    DisconnecterStage disconnectorStage;
    ...
    std::thread commandThread([&]() {
     ...
     std::thread DisconnectorThread([&]() {
        disconnectorStage.execute();  		// Start Disconnector stage
    });
    ...
    while (true) {
        ...
        std::thread([client_fd, &communicationStage, &commandExecuteStage, &disconnectorStage]() {
            while (true) {
                // Receive data from the client
                std::string receivedData = communicationStage.handleReceiveData(client_fd);
                if (receivedData == "exit\n") {
                    disconnectorStage.disconnectClient(client_fd);
                    break;
                }
